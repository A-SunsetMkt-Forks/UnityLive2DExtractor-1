...
namespace UnityLive2DExtractor
{
    ...
    {
        public static void ExtractLive2D(IGrouping<string, AssetStudio.Object> modelAssets, string destPath, string modelName, Live2DMotionMode motionMode, bool forceBezier)
        {
            ...
            if (motionMode == Live2DMotionMode.MonoBehaviour && fadeMotionList.Count > 0)  //motion from MonoBehaviour
            {
                ...
                foreach (var fadeMotionMono in fadeMotionList)
                {
                    ...
                    {
                        Console.WriteLine($"{"[Warning]".Color(brightYellow)} Fade motion \"{fadeMotionMono.m_Name}\" is not readable.");
                        continue;
                    }
            ...
            if (motions.Count == 0)
            {
                Console.WriteLine($"{"[Warning]".Color(brightYellow)} No motions found for \"{modelName}\" model.");
            }
            ...
            foreach (var monoBehaviour in expressionList)
            {
                ...
                {
                    Console.WriteLine($"{"[Warning]".Color(brightYellow)} Expression \"{expressionName}\" is not readable.");
                    continue;
                }
